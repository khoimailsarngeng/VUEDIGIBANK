  ...
  <q-layout view="hHh LpR fFf">
    <q-header class="text-grey-4 db-toolbar" height-hint="98">
      <q-toolbar>
        ...
        <q-btn-dropdown stretch flat icon="language" label="ເລືອກພາສາ">
          <q-list>
        ...
        <q-input outlined v-if="!smallScreen" :dark="true" v-model="searchQuery" debounce="500" dense filled placeholder="Search" clearable class="col-md-3">
          ...
          <template v-slot:label>
            <div class="row items-center no-wrap">
              ...
              <div class="text-center q-pa-sm">
                {{$store.state.authentication.authData.EMPNAME}}
              </div>
            </div>
          </template>
          <user-info></user-info>
    ...
    <menu-drawer :mini.sync="menuMini" :auto-expand="menuAutoExpand" :width="lang === 'la' ? 200 : 230">
      <template v-slot:menu>
        <menu-list :menu-items="CustomMenuLeft" v-on:expansionItemClick="menuMini = false"></menu-list>
      </template>
    </menu-drawer>
...
export default {
  ...
  data() {
    return {
      ...
      testColors: null,
      menuItems: [
        // { id: '1', title: 'Dashboard', link: '/home', icon: 'dashboard' },
        // { id: '11', title: 'Menu', link: '/menu', icon: 'menu' },
        // { id: '12', title: 'Role', link: '/role', icon: 'supervised_user_circle' }
        //   id: 'observability',
        //   title: 'Total Dashboards',
        //   icon: 'mdi-eye-check',
        //   caption: 'Monitor your process',
        //   items: [
        ...
        //   title: 'Setting',
        //   icon: 'settings',
        //   items: [{ id: '11', title: 'Menu', link: '/menu', icon: 'menu' },
        //     { id: '12', title: 'Role', link: '/role', icon: 'supervised_user_circle' }
        //   ]
        // }
  ...
  computed: {
    ...mapState({
      ...
      menuleft: state => state.menu.menuleft
    }),
    dark: {
      get() {
    ...
    CustomMenuLeft(){
      var aa = [];
      this.menuleft.forEach(element => {
        var a = {
          id: element.MENU_ID,
          title: this.lang == 'la' ? element.MENU_NAME_LA
          : this.lang == 'en' ? element.MENU_NAME_EN
          : element.MENU_NAME_VI,
          link : element.MENU_PATCH,
  ...
  watch: {
    ...
    lang(lang) {
      ...
      localStorage.setItem("lvbKPIDasboardlanguage", lang);
      this.setLanguage(lang);
    },
    leftShown(val){
        this.setMenuMini({ menuMini: !val });
    }
  },
  mounted() {
    //this.initialize();
    this.$i18n.locale = localStorage.getItem("lvbKPIDasboardlanguage")
      ? localStorage.getItem("lvbKPIDasboardlanguage")
      : this.$i18n.locale;
